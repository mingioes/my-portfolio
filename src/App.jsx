...
const projects = [
  {
    ...
    bullets: [
      "여행·일상 기록 시 사진과 함께 음성 메모를 남기는 기능을 위해 Whisper 모델을 ONNX로 변환하고 ONNX Runtime 기반 추론 동작 검증 수행, 온디바이스 적용 가능성 검토 (최종 앱 미반영)",
      "MobileNet · YOLOv8 추론 결과를 자연어 응답으로 제공하기 위한 LLM 연동 구조 검토",
      "Qualcomm AI Hub의 TFLite 양자화 모델(ESRGAN 등) 활용 및 온디바이스 환경 통합, NPU 파라미터 튜닝으로 이미지 업스케일링 품질 최적화",
      "SDK 호환성 이슈 10여 건을 4단계 트래킹 프로세스로 체계화 → 팀 공유 문서 관리 및 동일 오류 중복 조사 시간 감소",
      "Qualcomm 본사 엔지니어와 영어 Slack 채널을 통한 기술 질의 및 요구사항 조율",
    ...
    star: [
      ...
      { label: "A", title: "추론 파이프라인 설계 & 모델 최적화", body: "음성 인식(Whisper)의 ONNX 추론 동작을 검증하고, 객체 탐지(YOLOv8·MobileNet) 및 이미지 업스케일링(ESRGAN) 모델을 Snapdragon 8 Gen 3 환경에 적용했습니다. NPU 파라미터 튜닝을 통해 객체 탐지 성능을 개선했고, CLIP 기반 이미지-텍스트 임베딩으로 자연어 사진 검색 기능을 구현했습니다. SDK 호환성 이슈는 4단계 트래킹 체계로 관리하며 Qualcomm 본사에 영어로 질의해 해결했습니다." },
